...
__global__ void detMatrixOnGPUMix(float *matrix, int nx, int ny){
    ...
    if((pivot > -0.00001) && (pivot < 0.00001)){
        ...
        //printf(" pivot de dentro: %f \n",pivot);
		int i = idxPivot;
		bool newpivot_found = false;
        while(!newpivot_found){
            //printf("i: %d , Matrix[i]: %f \n", i, matrix[i]);
			i = i + matrix_size;
			if(matrix[i] != 0){
				newpivot_found = true;
                printf("index i = %d new pivot founded matrix[i]= %f (i-1)/matrix_size = %f \n", i, (i-1)/matrix_size);
				...
				for(int k = idxCollumn; k < idxCollumn + matrix_size; k++) {
					...
					matrix[i] = line[k-1];
                    i++;
                    printf("no for: matrix[k] = %f, matrix[i] = %f", matrix[k],  matrix[i]);
				}
			}
		}
    }
